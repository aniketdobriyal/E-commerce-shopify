...
export default function Men() {
  ...
  const mensProducts = useMemo(() => {
    ...
    return productsFromContext.filter((item) => {
      ...
      return cat === "men" || cat === "men's clothing";
    });
  }, [productsFromContext]);
          ...
          {isLoading
            ? [...Array(8)].map((_, idx) => (
                <Col xs={6} sm={6} md={4} lg={3} key={idx}>
                  <Card className="shadow-sm border-0 rounded-4 w-100 h-100 position-relative">
                    <Badge bg="secondary" text="light" className="position-absolute" style={{ top: "10px", left: "10px", zIndex: 10 }}>
                      Loading
                    </Badge>
                    <div style={{ height: "200px", display: "flex", justifyContent: "center", alignItems: "center", backgroundColor: "#f8f9fa", borderTopLeftRadius: "12px", borderTopRightRadius: "12px", overflow: "hidden" }}>
            ...
            ? mensProducts.map((item) => (
                <Col xs={6} sm={6} md={4} lg={3} key={item.id} className="d-flex align-items-stretch m-0 p-0">
                  ...
                  >
                    <Badge bg="success" className="position-absolute" style={{ top: "10px", left: "10px", zIndex: 10 }}>Men</Badge>

                    <div style={{ height: "200px", display: "flex", justifyContent: "center", alignItems: "center", backgroundColor: "", padding: "15px", borderTopLeftRadius: "12px", borderTopRightRadius: "12px", overflow: "hidden" }}>
                      <Card.Img src={item.image} alt={item.title} style={{ maxHeight: "150px", maxWidth: "100%", objectFit: "contain" }} />
                    </div>

                    <Card.Body className="text-center d-flex flex-column">
                      {/* Title */}
                      <Card.Title className="fs-6 fw-bold text-truncate mb-1" title={item.title}>{item.title}</Card.Title>

                      {/* Multi-line description */}
                      <Card.Text className="text-muted mb-2" style={{ fontSize: "0.85rem", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", textOverflow: "ellipsis" }} title={item.description}>
